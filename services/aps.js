...
service.translateObject = async (urn, token) => {
    const job = {
        input: { urn },
        output: { destination: {region: "us"},formats: [{type: "ifc"}]}
    };
    const resp = await new APS.DerivativesApi().translate(job, { xAdsForce: true }, internalAuthClient, token);
    return resp.body;
};

service.getManifest = async (urn, token) => {
    try {
        const resp = await new APS.DerivativesApi().getManifest(urn, {}, internalAuthClient, token);
        return resp.body;
    } catch (err) {
...
service.getDerivativeManifest = async (urn, derivativeUrn, token) => {
    try {
        const resp = await new APS.DerivativesApi().getDerivativeManifest(urn, derivativeUrn, {}, internalAuthClient, token);
        return resp;
    } catch (err) {
        if (err.response.status === 404) {
            return null;
