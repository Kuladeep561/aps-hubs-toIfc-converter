...
async function getJSON(url) {
    try {
        ...
        console.error(error);
        alert('An error occurred while fetching data. See the console for more details.');
        return [];
    }
}
...
async function extractDerivativeUrnForIfc(responseBody) {
    ...
    if (ifcDerivative) {
        const ifcChild = ifcDerivative.children.find((child) => child.role === 'ifc');
...
function startAutoRefresh(viewer, urn) {
    autoRefreshInterval = setInterval(() => getManifest(viewer, urn), 65000); // Refresh every 65 seconds
}
...
async function getDerivativeManifest(urn, derivativeUrn) {
    try {
        ...
        const derivativeManifest = await getJSON(`/api/designdata/${urn}/manifest/${encodedDerivativeUrn}`);
        return derivativeManifest;
    } catch (error) {
        alert('Could not fetch the derivative manifest. See the console for more details.');
        console.error(error);
    }
}
...
async function getManifest(viewer, urn) {
    try {
        ...
        switch (status.status) {
            ...
            default:
                ...
                if (derivativeUrn) {
                    ...
                    const derivativeManifestResponse = await getDerivativeManifest(urn, derivativeUrn);
        ...
        alert('An error occurred while fetching the manifest. See the console for more details.');
        console.error(error);
    }
}
...
export async function translateToIfc(data, viewer) {
    ...
    try {
        const response = await fetch('/api/designdata/job', { method: 'POST', body: data });
        ...
        console.error(error);
        alert('An error occurred during translation. See the console for more details.');
    }
}
